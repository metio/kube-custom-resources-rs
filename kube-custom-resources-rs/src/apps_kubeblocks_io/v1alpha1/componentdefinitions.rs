...
pub struct ComponentDefinitionSpec {
    ...
    ///  - The ConfigMap containing the scripts. - The mount point where the scripts will be mounted inside the container. 
    ///  This field is immutable.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    ///  - Use well-established and widely recognized protocol names or service types. - Ensure that the `serviceKind` accurately represents the primary service type offered by the Component. - If the Component provides multiple services, choose the most prominent or commonly used protocol. - Limit the `serviceKind` to a maximum of 32 characters for conciseness and readability. 
