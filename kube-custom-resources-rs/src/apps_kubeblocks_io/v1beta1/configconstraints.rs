...
// kopium command: kopium --docs --filename=./crd-catalog/apecloud/kubeblocks/apps.kubeblocks.io/v1beta1/configconstraints.yaml --derive=Default --derive=PartialEq --smart-derive-elision
...
pub struct ConfigConstraintSpec {
    ...
    ///  Example: ``` fileFormatConfig: format: ini iniConfig: sectionName: mysqld ```
    ...
    ///  Example: ```yaml dynamicReloadAction: tplScriptTrigger: namespace: kb-system scriptConfigMapRef: mysql-reload-script sync: true ```
