...
// kopium command: kopium --derive Default --derive PartialEq --docs --filename ./crd-catalog/clusternet/clusternet/apps.clusternet.io/v1alpha1/subscriptions.yaml
...
pub struct SubscriptionStatusAggregatedStatusesFeedStatusDetailsReplicaStatus {
    ...
    /// currentReplicas is the number of Pods created by the workload controller from the StatefulSet version indicated by currentRevision.
    ...
    /// Total number of unavailable pods targeted by this workload. This is the total number of pods that are still required for the workload to have 100% available capacity. They may either be pods that are running but not yet available or pods that still have not been created.
