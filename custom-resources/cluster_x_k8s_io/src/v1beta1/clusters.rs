...
// kopium command: kopium --docs --derive=Default --derive=PartialEq --smart-derive-elision --filename crd-catalog/kubernetes-sigs/cluster-api/cluster.x-k8s.io/v1beta1/clusters.yaml
...
pub struct ClusterTopologyControlPlaneMachineHealthCheck {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "unhealthyMachineConditions")]
    pub unhealthy_machine_conditions: Option<Vec<ClusterTopologyControlPlaneMachineHealthCheckUnhealthyMachineConditions>>,
...
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq)]
pub struct ClusterTopologyControlPlaneMachineHealthCheckUnhealthyMachineConditions {
    /// status of the condition, one of True, False, Unknown.
    pub status: ClusterTopologyControlPlaneMachineHealthCheckUnhealthyMachineConditionsStatus,
...
/// status for at least the timeout value, a machine is considered unhealthy.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq)]
pub enum ClusterTopologyControlPlaneMachineHealthCheckUnhealthyMachineConditionsStatus {
    True,
    False,
    Unknown,
}
...
pub struct ClusterTopologyWorkersMachineDeploymentsMachineHealthCheck {
    ...
    /// logical OR, i.e. if any of the conditions is met, the machine is unhealthy.
    #[serde(default, skip_serializing_if = "Option::is_none", rename = "unhealthyMachineConditions")]
    pub unhealthy_machine_conditions: Option<Vec<ClusterTopologyWorkersMachineDeploymentsMachineHealthCheckUnhealthyMachineConditions>>,
    /// unhealthyRange specifies the range of unhealthy machines allowed.
    /// Any further remediation is only allowed if the number of machines selected by "selector" as not healthy
    /// is within the range of "unhealthyRange". Takes precedence over maxUnhealthy.
    /// Eg. "[3-5]" - This means that remediation will be allowed only when:
...
/// status for at least the timeout value, a machine is considered unhealthy.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq)]
pub struct ClusterTopologyWorkersMachineDeploymentsMachineHealthCheckUnhealthyMachineConditions {
    /// status of the condition, one of True, False, Unknown.
    pub status: ClusterTopologyWorkersMachineDeploymentsMachineHealthCheckUnhealthyMachineConditionsStatus,
    /// timeout is the duration that a Machine must be in a given status for,
    /// after which the Machine is considered unhealthy.
    /// For example, with a value of "1h", the Machine must match the status
    /// for at least 1 hour before being considered unhealthy.
...
/// status for at least the timeout value, a machine is considered unhealthy.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq)]
pub enum ClusterTopologyWorkersMachineDeploymentsMachineHealthCheckUnhealthyMachineConditionsStatus {
    True,
    False,
    Unknown,
}
